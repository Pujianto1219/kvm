...
function listssh(){
    ...
    # Cek Durasi Lock
    if [ -f /etc/waktulockssh ]; then
        durasi_lock=$(cat /etc/waktulockssh)
    else
        durasi_lock="-"
    fi
    ...
    if [[ $lock == "0" ]]; then
        ...
        read -rp "│ Durasi Penguncian (Menit) : " -e notif2
        echo "${notif2}" > /etc/waktulockssh
        
        echo -e "$COLOR1│${NC}"
        echo -e "$COLOR1│${NC} Tentukan batas maksimal peringatan (notifikasi)"
        echo -e "$COLOR1│${NC} sebelum akun terkunci otomatis (Misal: 3)"
        read -rp "│ Batas Peringatan Multi Login : " -e notif
        ...
        echo -e "$COLOR1│${NC} ${WH}Batas Notif : $notif Peringatan${NC}"
        echo -e "$COLOR1│${NC} ${WH}Durasi Lock : $notif2 Menit${NC}"
        echo -e "$COLOR1╰═════════════════════════════════════════════════╯${NC}"
        ...
        echo -e "$COLOR1│${NC} terhadap user yang melanggar (Misal: 5)"
        read -rp "│ Waktu Scan (Menit) : " -e notif2
        
        # Setting Cronjob Auto Delete
        ...
        echo -e "$COLOR1│${NC}"
        echo -e "$COLOR1│${NC} Tentukan batas maksimal peringatan (notifikasi)"
        echo -e "$COLOR1│${NC} sebelum akun dihapus permanen (Misal: 3)"
        read -rp "│ Batas Peringatan Multi Login : " -e notif
        echo "$notif" > /etc/xray/sshx/notif
