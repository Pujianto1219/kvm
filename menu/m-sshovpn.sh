...
# Mengubah path public_html menjadi /var/www/html agar terbaca oleh Nginx/Apache bawaan
cat > /var/www/html/ssh-$Login.html <<-END
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    ...
    <style>body { background-color: #171b26; color: #9ba1b4; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; display: flex; justify-content: center; padding: 20px; margin: 0; } .container { background-color: #212634; border-radius: 12px; width: 100%; max-width: 400px; padding: 25px; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.3); } .header { display: flex; align-items: center; margin-bottom: 30px; } .header-icon { background-color: #1b4933; color: #22c55e; width: 50px; height: 50px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 24px; margin-right: 15px; } .header-text h2 { margin: 0; color: #ffffff; font-size: 18px; } .header-text p { margin: 5px 0 0; font-size: 13px; } .section-title { display: flex; align-items: center; color: #ffffff; font-size: 16px; font-weight: 600; margin: 25px 0 15px; } .section-title i { margin-right: 10px; } .text-green { color: #22c55e; } .text-purple { color: #c084fc; } .row { display: flex; justify-content: space-between; align-items: center; padding: 12px 0; border-bottom: 1px solid #2d3345; font-size: 14px; } .row:last-child { border-bottom: none; } .row-label { display: flex; align-items: center; } .row-label i { margin-right: 10px; font-size: 12px; color: #6b7280; width: 15px; text-align: center; } .row-value { color: #e2e8f0; text-align: right; word-break: break-all; margin-left: 15px; } .pubkey-container { display: flex; align-items: center; background-color: #2d3345; padding: 8px; border-radius: 8px; border: 1px solid #3f4760; } .pubkey-text { font-family: monospace; font-size: 11px; color: #e2e8f0; max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; } .btn-copy { background-color: #3b82f6; color: white; border: none; border-radius: 6px; padding: 8px 12px; margin-left: 10px; cursor: pointer; font-size: 14px; } .status-unlocked { color: #22c55e; font-weight: 600; }</style>
</head>
<body>
<div class="container">
    <div class="header">
        <div class="header-icon"><i class="fas fa-user-shield"></i></div>
        <div class="header-text"><h2>Account SSH/OpenVPN</h2><p>$domen</p></div>
    </div>
    <div class="section-title"><i class="fas fa-key text-green"></i> Credentials</div>
    <div class="row"><div class="row-label"><i class="fas fa-user"></i> Username</div><div class="row-value">$Login</div></div>
    <div class="row"><div class="row-label"><i class="fas fa-lock"></i> Password</div><div class="row-value">$Pass</div></div>
    ...
    <div class="row"><div class="row-label"><i class="fas fa-server"></i> NS Domain</div><div class="row-value">$sldomain</div></div>
    <div class="row"><div class="row-label"><i class="fas fa-toggle-on"></i> Status</div><div class="row-value status-unlocked">Unlocked</div></div>
    <div class="section-title"><i class="fas fa-plug text-purple"></i> Port Settings</div>
    <div class="row"><div class="row-label"><i class="fas fa-shield-alt"></i> OpenVPN</div><div class="row-value">1194, 2200</div></div>
    <div class="row"><div class="row-label"><i class="fas fa-tachometer-alt"></i> SlowDNS</div><div class="row-value">53, 443</div></div>
    <div class="row"><div class="row-label"><i class="fas fa-random"></i> UDP Custom</div><div class="row-value">1-65535</div></div>
    <div class="row"><div class="row-label"><i class="fas fa-water"></i> Squid</div><div class="row-value">3128</div></div>
    <div class="row"><div class="row-label"><i class="fas fa-robot"></i> BadVPN</div><div class="row-value">7100, 7300</div></div>
    <div class="section-title"><i class="fas fa-network-wired text-purple"></i> Connection</div>
    <div class="row"><div class="row-label"><i class="fas fa-mobile-alt"></i> Device Limit</div><div class="row-value">${info_ip}</div></div>
    <div class="row"><div class="row-label"><i class="fas fa-calendar-alt"></i> Expired</div><div class="row-value">$exp</div></div>
</div>
<script>function copyKey() { var keyText = document.getElementById("pubkey").innerText; navigator.clipboard.writeText(keyText).then(function() { alert("Pubkey disalin!"); }); }</script>
</body>
</html>
END

# Menghapus port 81 pada link Save Account di Telegram
TEXT="
🧿───────────────────🧿            
        ✨PREMIUM SSH✨
🔹 Informasi Akun Anda
...
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/xray/sshx/akun/log-create-${Login}.log
echo -e "$COLOR1 ${NC} ${WH}Format Web View  ${COLOR1}:" | tee -a /etc/xray/sshx/akun/log-create-${Login}.log
# Menghapus port 81 pada log output di terminal
echo -e "$COLOR1 ${NC} ${WH}https://$domen/ssh-$Login.html${NC}" | tee -a /etc/xray/sshx/akun/log-create-${Login}.log
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/xray/sshx/akun/log-create-${Login}.log
...
# =========================================================
# 1. GENERATE BACKEND LIMIT IP (CLEAN & SYSTEMD BASE)
# =========================================================
cat > /usr/local/bin/limit-ip << 'EOF'
#!/bin/bash
# Backend Auto Limit IP (Multi-Login) SSH & Dropbear by AcilShop

DIR_CONF="/etc/xray/sshx"
LOG_FILE="${DIR_CONF}/limit-ip.log"
mkdir -p $DIR_CONF
...
while true; do
    # Ambil daftar semua user SSH yang valid di sistem
    mapfile -t users < <(awk -F: '$3 >= 1000 && $1 != "nobody" {print $1}' /etc/passwd)
    
    for usr in "${users[@]}"; do
        limitip=$(cat ${DIR_CONF}/${usr}IP 2>/dev/null || echo "0")
        
        # Skip jika unlimited atau belum disetting
        if [[ "$limitip" -eq 0 ]] || [[ "$limitip" -eq 9999 ]]; then
            continue
        fi

        # Deteksi jumlah sesi dari proses OpenSSH & Dropbear yang berjalan
        cek_ssh=$(ps aux | grep -w "sshd: $usr" | grep -v grep | wc -l)
        cek_db=$(ps -u "$usr" 2>/dev/null | grep dropbear | grep -v grep | wc -l)
        login_count=$((cek_ssh + cek_db))
        
        # EKSEKUSI JIKA MELEBIHI LIMIT IP
        if [ "$login_count" -gt "$limitip" ]; then
            # Cek status akun, jika sudah dilock jangan dieksekusi ulang
            status=$(passwd -S "$usr" 2>/dev/null | awk '{print $2}')
            if [[ "$status" != "L" && "$status" != "LK" ]]; then
                
                # Sanksi: Tendang Sesi & Kunci Akun
                killall -u "$usr" &>/dev/null
                usermod -L "$usr" &>/dev/null
                
                # Catat ke Database Log
                echo "[$(date '+%Y-%m-%d %H:%M:%S')] 🔒 $usr (Limit IP: $limitip | Sesi Aktif: $login_count)" >> $LOG_FILE
                
                # Kirim Notifikasi Telegram
                if [ -n "$KEY" ] && [ -n "$CHATID" ]; then
                    TEXT="
<code>◇━━━━━━━━━━━━━━━━━◇</code>
<b> 🚨 AUTO LOCK MULTI LOGIN </b>
<code>◇━━━━━━━━━━━━━━━━━◇</code>
<b>DOMAIN   :</b> <code>${domen}</code>
<b>USERNAME :</b> <code>${usr}</code>
<b>LIMIT IP :</b> <code>${limitip} Device</code>
<b>TERDETEKSI :</b> <code>${login_count} Device Aktif</code>
<b>SANKSI   :</b> <b>AKUN DIKUNCI</b>
<code>◇━━━━━━━━━━━━━━━━━◇</code>
<i>Sistem otomatis memutus akses.</i>"
    ...
    # Rilekskan CPU selama 30 detik sebelum loop berikutnya
    sleep 30
done
EOF
...
# =========================================================
# 3. TAMPILAN MENU & LOGIKA KONTROL
# =========================================================
# Cek Status Daemon
if systemctl is-active --quiet limit-ip; then
...
echo -e "$COLOR1╭───────────────────────────────────────────╮${NC}"
echo -e "$COLOR1│${NC}         ${WH}• AUTO LIMIT IP (SSH) •${NC}           $COLOR1│${NC}"
echo -e "$COLOR1├───────────────────────────────────────────┤${NC}"
echo -e "$COLOR1│${NC} Status Sistem : $status_service             "
echo -e "$COLOR1├───────────────────────────────────────────┤${NC}"
echo -e "$COLOR1│${NC} ${WH}[1] Turn ON Auto Limit IP${NC}                 $COLOR1│${NC}"
echo -e "$COLOR1│${NC} ${WH}[2] Turn OFF Auto Limit IP${NC}                $COLOR1│${NC}"
echo -e "$COLOR1│${NC} ${WH}[3] Lihat User Terkunci & Buka Kunci${NC}      $COLOR1│${NC}"
...
case $opt in
    1)
        systemctl enable --now limit-ip &> /dev/null
        echo -e " Backend Limit IP berhasil diaktifkan."
        sleep 1
        toggle_autokick_ssh
        ;;
    2)
        systemctl disable --now limit-ip &> /dev/null
        echo -e " Backend Limit IP berhasil dimatikan."
        sleep 1
        toggle_autokick_ssh
        ;;
    3)
        clear
        echo -e "$COLOR1╭───────────────────────────────────────────────────╮${NC}"
        echo -e "$COLOR1│${NC}             ${WH}DATA USER TERKUNCI (LOCKED)${NC}           $COLOR1│${NC}"
        echo -e "$COLOR1├───────────────────────────────────────────────────┤${NC}"
        printf "$COLOR1│${NC} %-15s │ %-8s │ %-19s $COLOR1│${NC}\n" "Username" "Status" "Penyebab Kunci"
        echo -e "$COLOR1├───────────────────────────────────────────────────┤${NC}"

        mapfile -t users < <(awk -F: '$3 >= 1000 && $1 != "nobody" {print $1}' /etc/passwd)
        found=0
        for usr in "${users[@]}"; do
            # Cek status password user langsung di kernel
            status=$(passwd -S "$usr" 2>/dev/null | awk '{print $2}')
            
            # Jika statusnya "L" (Locked)
            if [[ "$status" == "L" || "$status" == "LK" ]]; then
                # Membaca log untuk mencari tahu apa yang mengunci user ini
                if grep -q "$usr" /etc/xray/sshx/limit-ip.log 2>/dev/null; then
                    ket="Multi-Login IP"
                elif grep -q "$usr" /etc/xray/sshx/quota-lock.log 2>/dev/null; then
                    ket="Over Quota"
                else
                    ket="Manual Lock Admin"
                fi
                printf "$COLOR1│${NC} %-15s │ \033[0;31m%-8s\033[0m │ %-19s $COLOR1│${NC}\n" "$usr" "LOCKED" "$ket"
                found=1
            fi
        done

        if [ $found -eq 0 ]; then
            echo -e "$COLOR1│${NC}            \033[0;32mBelum ada user yang dilimit\033[0m            $COLOR1│${NC}"
            echo -e "$COLOR1╰───────────────────────────────────────────────────╯${NC}"
            echo ""
            read -n 1 -s -r -p " Press any key to back..."
            toggle_autokick_ssh
            ...
            echo -e "$COLOR1╰───────────────────────────────────────────────────╯${NC}"
            echo ""
            echo -e " ${WH}Pilih Aksi:${NC}"
            echo -e " [1] Buka Kunci (Unlock) User"
            echo -e " [0] Kembali"
            read -p " Pilihan: " act
            if [ "$act" == "1" ]; then
                read -p " Masukkan Username yang ingin di-unlock: " unlock_user
                if id "$unlock_user" &>/dev/null; then
                    # Proses buka kunci
                    usermod -U "$unlock_user" &>/dev/null
                    
                    # Bersihkan jejak di log agar limit-ip tidak salah baca
                    sed -i "/$unlock_user/d" /etc/xray/sshx/limit-ip.log 2>/dev/null
                    sed -i "/$unlock_user/d" /etc/xray/sshx/quota-lock.log 2>/dev/null
                    
                    echo -e " \033[0;32mSukses!\033[0m Akun '$unlock_user' berhasil dibuka dan siap digunakan."
                else
                    echo -e " \033[0;31mGagal!\033[0m Username '$unlock_user' tidak ditemukan di sistem."
                fi
                sleep 2
                toggle_autokick_ssh
            else
