...
until [[ $masaaktif =~ ^[0-9]+$ ]]; do read -p "  Expired (Hari) : " masaaktif; done
...
│Encryption : none
│Network    : WS or gRPC
│Path       : <code>/vless</code>
│Service    : <code>vless-grpc</code>
└─────────────────────
🫧Link Vless TLS:
...
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1${NC}${WH}Username   ${COLOR1}: ${WH}${user}" | tee -a /etc/vless/akun/log-create-${user}.log
...
echo -e "$COLOR1${NC}${WH}Limit IP   ${COLOR1}: ${WH}${info_ip}" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1${NC}${WH}Quota Limit${COLOR1}: ${WH}${info_quota}" | tee -a /etc/vless/akun/log-create-${user}.log
...
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1${NC}${COLOR1}Link TLS  ${WH}:    ${vlesslink1}${NC}" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1${NC}${COLOR1}Link NTLS ${WH}:    ${vlesslink2}${NC}" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1${NC}${COLOR1}Link GRPC ${WH}:    ${vlesslink3}${NC}" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/vless/akun/log-create-${user}.log
...
# Akan menghapus inject Config Xray Vless, File IP, File Quota, File Txt, lalu Restart Xray
# ==========================================
echo "sed -i '/^#vl $user $exp $uuid/,/^},{/d' /etc/xray/config.json && sed -i '/^#vlg $user $exp/,/^},{/d' /etc/xray/config.json && rm -f /etc/vless/${user}IP && rm -f /etc/vless/${user} && rm -f /home/vps/public_html/vless-${user}.txt && systemctl restart xray" | at now + $timer minutes &> /dev/null
# ==========================================
...
echo -e "$COLOR1${NC}${WH}City       ${COLOR1}: ${WH}$CITY" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1${NC}${WH}Domain     ${COLOR1}: ${WH}${domain}" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1${NC}${WH}Limit IP   ${COLOR1}: ${WH}${iplim} IP" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1${NC}${WH}Limit Quota${COLOR1}: ${WH}${Quota} GB" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1 ${NC} ${WH}Expired In ${COLOR1}: ${WH}$timer Minutes" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1${NC}${WH}Port TLS      ${COLOR1}: ${WH}443" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1${NC}${WH}Port NTLS    ${COLOR1}: ${WH}80,8080" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1${NC}${WH}Port gRPC     ${COLOR1}: ${WH}443" | tee -a /etc/vless/akun/log-create-${user}.log
